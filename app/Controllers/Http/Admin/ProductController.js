...
class ProductController {
	...
	async update({ params: { id }, request, response }) {
		...
		try {
			const { name, description, price, image_id } = request.all()
			/*
			 *atualiza (merge) o obj product
			 *com os dados de request.all() {name, description, price, image_id}
			 */
	...
	async destroy({ params: { id }, response }) {
		...
		try {
			...
			return response.status(500).send({ message: "Não efetuar esta operação!" })
		}
	}
}
